...
class RainbowDQn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (val - adv.mean(1).unsqueeze(1)).expand(x.size(0), self.num_actions) + adv
        return x
        # return adv
...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_dqn:
            ...
            ###########################
            # YOUR IMPLEMENTATION HERE #
            model_path = "models/DDQN_WE_20.pth"
    ...
    def make_action(self, observation, test=True):
        ...
        # YOUR IMPLEMENTATION HERE #
        if test:
            observation = torch.from_numpy(np.transpose(np.asarray(observation,dtype=np.float32)/255,(2,0,1))).unsqueeze(0)
    ...
    def optimize_step(self):
        if(len(self.memory) < 10*self.batchsize):
            return
       ...
       # for i in range(self.batchsize):
        ...
        plcy_act = torch.argmax(self.policy_net(next_states),dim=1)[0].detach()
        next_state_intm_val = self.target_net(next_states)
    ...
    def train(self):
        ...
        for ne in range(self.n_episodes):
            ...
            if ne%100==0:
                ...
                wandb.log({"episode":ne,"epsilon":epsilon_val,"episode_reward": ep_r/100,"max_reward": max_ep_r})
                ep_r = 0
            # if ne%1000==0:
                # self.target_net.load_state_dict(self.policy_net.state_dict())
            if ne%5000==0:
                model_save_path = "models/DDQN_RO_%d.pth" %(ne//5000)
                try:
